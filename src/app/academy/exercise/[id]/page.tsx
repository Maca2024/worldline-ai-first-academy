...
import { ArrowLeft, Send, Bot, CheckCircle2, Save } from 'lucide-react';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.status === 401) {
        setResult({
          score: 0,
          feedback: 'Je sessie is verlopen. Log opnieuw in om door te gaan.',
          strengths: [],
          improvements: [],
          saved: false,
        });
      ...
      setResult({
        score: 0,
        feedback: 'Kan geen verbinding maken. Probeer het opnieuw.',
        strengths: [],
        improvements: [],
        saved: false,
      });
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm text-gray-400">{exercise.points} punten</span>
      ...
      {/* Submission — hide after successful submit */}
      {!result && (
        <Card>
          <CardContent>
            ...
            <div className="flex items-center justify-between mt-4">
              <p className="text-xs text-gray-500">{submission.length}/10.000 tekens</p>
              <Button
        ...
        <Card className="border-accent-purple/20">
          <CardContent>
            <div className="flex items-center gap-3 mb-4">
              ...
              <Badge variant={result.score >= 70 ? 'green' : result.score >= 40 ? 'blue' : 'coral'}>
            ...
            <div className="flex items-center justify-between border-t border-white/5 pt-3 mt-2">
              <p className="text-xs text-gray-500">
                AI-gegenereerde evaluatie. Je instructor kan aanvullend beoordelen.
              </p>
              <Button
                variant="ghost"
                size="sm"
                onClick={() => { setResult(null); setSubmission(''); }}
              >
                Opnieuw proberen
              </Button>
            </div>
